...
SAVE_FOLDER = "../data/triplet_visualization"
...
for game in GAMES:
    ...
    # -------------------------------------------------
    # Score triplets (normalized margin)
    # -------------------------------------------------
    ...
    print(f"Using top {n_triplets} triplets (20%)")
    ...
    # -------------------------------------------------
    # Run t-STE multiple times (same subset)
    # -------------------------------------------------
    best_score = -np.inf
    ...
    for idx, (i, j, k) in enumerate(tqdm(selected_triplets)):
        ...
        clip_i = index_to_clip[i]
        clip_j = index_to_clip[j]
        clip_k = index_to_clip[k]

        src_i = os.path.join(CLIP_BASE_FOLDER, game, "human_dqn_visualitzation", clip_i)
        src_j = os.path.join(CLIP_BASE_FOLDER, game, "human_dqn_visualitzation", clip_j)
        src_k = os.path.join(CLIP_BASE_FOLDER, game, "human_dqn_visualitzation", clip_k)
