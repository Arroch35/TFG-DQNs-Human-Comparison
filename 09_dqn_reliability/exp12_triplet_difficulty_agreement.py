...
# MAIN LOOP
# =========================================================
summary_rows = []   # <- for the new summary CSV
...
for game in GAMES:
    ...
    for seed in SEEDS:
        ...
        if seed == "seed_42":
            ...
            sorted_rel    = np.argsort(filtered_diff)
    ...
    if master_df is not None:
        ...
        # ── Per-difficulty summary (mean±std agreement + score range) ──
        ...
        if answer_cols:
            for diff in ["hard_triplets", "medium_triplets", "easy_triplets", "outside_buckets"]:
                ...
                std_agree  = sub["frac_seeds_agree"].std()    # <- std across triplets

                score_min  = sub["difficulty_score"].min()
                score_max  = sub["difficulty_score"].max()

                print(f"    {diff}: {mean_agree:.3f} ± {std_agree:.3f}  "
                      f"(score range [{score_min:.3f}, {score_max:.3f}],  n={len(sub)})")
...
summary_csv = os.path.join(OUTPUT_DIR, "triplet_agreement_summary.csv")
