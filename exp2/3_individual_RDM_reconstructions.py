...
rdm_dir = "../data/triplets_results/exp2/cleaned_results/rdms_human_experiment_rsa" #"../data/rdms_human_experiment_rsa"
...
for game in games:
    ...
    for participant_id, participant_df in df.groupby("participant_id"):
        triplets = np.ascontiguousarray(participant_df[triplet_cols].values, dtype=np.int32)

        print(f"\nTraining t-STE for {game} | participant {participant_id} ({n_clips} clips, {len(triplets)} triplets)")
        # -------------------------------
        ...
        # Save RDM as .npy
        # -------------------------------
        rdm_file = os.path.join(rdm_dir, f"{game}_participant_{participant_id}_rdm.npy")
        np.save(rdm_file, rdm_matrix)
        print(f"Saved RDM for {game} | participant {participant_id} to {rdm_file}")
        # -------------------------------
        ...
        plt.colorbar(im)
        plt.title(f"{game} | participant {participant_id} RDM (t-STE)")
        plt.xlabel("Clip")
        plt.ylabel("Clip")
        ...
        plt.yticks(ticks)
        plt.tight_layout()
        png_file = os.path.join(rdm_dir, f"{game}_participant_{participant_id}_rdm_heatmap.png")
